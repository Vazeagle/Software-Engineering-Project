...
def main():
    ...
    canvas = Canvas(container, bg="blue", width=getRes[0] - 50, height=getRes[1] - 70, borderwidth=0
    ...
    label_left = Label(init_Label, text="Simplified-DES Encryption\Decryption\n", bg="salmon1",
                       font=("Calibri", 24, "bold"))  # aristero menu
    label_l_up = Label(label_left, borderwidth=1, highlightthickness=0, bg="salmon1")  # photo parmenidi
    label_l_down = Label(label_left, borderwidth=1, highlightthickness=0,
                         bg="salmon1")  # button gia menu kai alla frames

    label_right = Label(init_Label, borderwidth=1, highlightthickness=0, bg="salmon1")  # dexio menu
    label_r_up = Label(label_right, borderwidth=1, highlightthickness=0,
                       bg="salmon1")  # panw meros me perilipsh kai hmerologio
    label_ru_up = Label(label_r_up, borderwidth=1, highlightthickness=0,
                        bg="salmon1")  # kalos orisate+ to eniaio susthma klp
    ...
    label_ruu_down = Label(label_ru_up, text="Το ενιαίο σύστημα για τις Πανελλήνιες Εξετάσεις.\n", borderwidth=0,
                           highlightthickness=0, bg="salmon1", font=("Calibri", 18))

    # isws na 8elei up kai oxi left
    label_ru_left = Label(label_r_up, borderwidth=1, highlightthickness=0,
                          bg="salmon1")  # foto parmenidi+ desription parmenidi
    label_rul_left = Label(label_ru_left, text="PHOTO PARMENIDI", borderwidth=1, highlightthickness=0,
                           bg="salmon1")  # foto parmenidi
    label_rul_right = Label(label_ru_left, borderwidth=1, highlightthickness=0,
                            bg="salmon1")  # Ο παρμενιδης ειναι bla bla

    label_ru_right = Label(label_r_up, borderwidth=0, highlightthickness=1,
                           bg="salmon1")  # hmerologio gramma + hmerologio
    label_rur_up = Label(label_ru_right, text="Ημερολόγιο\n", borderwidth=1, highlightthickness=0, bg="salmon",
                         font=("Calibri", 18))  # hmerologio gramma
    ...
    label_rd_up = Label(label_r_down, borderwidth=1, highlightthickness=0, bg="salmon1")  # sunoptiko profil
    label_rd_down = Label(label_r_down, borderwidth=1, highlightthickness=0, bg="salmon1")  # onoma xrhsth kai alla
    label_rdd_left = Label(label_rd_down, borderwidth=1, highlightthickness=0, bg="salmon1")  # eikona user
    label_rdd_right = Label(label_rd_down, borderwidth=1, highlightthickness=0,
                            bg="salmon1")  # onoma xrhsth status lukeio kai alla

    label_rddr_left = Label(label_rdd_right, borderwidth=1, highlightthickness=0,
                            bg="salmon1")  # onoma xrhsth kai status
    label_rddrl_up = Label(label_rddr_left, borderwidth=1, highlightthickness=0, bg="salmon1")  # onoma xrhsth
    label_rddrl_down = Label(label_rddr_left, borderwidth=1, highlightthickness=0, bg="salmon1")  # katastash
    label_rddrld_left = Label(label_rddrl_down, borderwidth=1, highlightthickness=0, bg="salmon1")  # katastash text
    label_rddrld_right = Label(label_rddrl_down, borderwidth=1, highlightthickness=0,
                               bg="salmon1")  # input katastashs apo data base

    label_rddr_right = Label(label_rdd_right, borderwidth=1, highlightthickness=0,
                             bg="salmon1")  # lukeio kai kateu8unsh
    ...
    label_rddrru_right = Label(label_rddrr_up, borderwidth=1, highlightthickness=0,
                               bg="salmon1")  # lukeio input from db
    ...
    descriptionText = Text(label_rul_right, height=7, bg="salmon1", borderwidth=0, highlightthickness=0,
                           font=("Calibri", 14), width=100)
    descriptionText.insert(INSERT,
                           "Ο Παρμενίδης ήταν αρχαίος έλληνας φιλόσοφος. Γεννήθηκε στην Ελέα της Μεγάλης Ελλάδας\n στα τέλητου 6ου αιώνα π.Χ., ....")
    ...
    butttonNext2 = Button(label_l_down, text="Εβδομαδιαίο Πρόγραμμα", command=lambda: raise_frame(intro_Frame)
                          , bg="SteelBlue3")
    butttonNext3 = Button(label_l_down, text="Αποτελέσματα", command=lambda: raise_frame(intro_Frame), bg="SteelBlue3")
    butttonNext4 = Button(label_l_down, text="Πανεπιστημιακά Ιδρύματα", command=lambda: raise_frame(intro_Frame)
                          , bg="SteelBlue3")
    butttonNext5 = Button(label_l_down, text="Πληροφορίες Χρήστη", command=lambda: raise_frame(intro_Frame)
                          , bg="SteelBlue3")
    butttonNext6 = Button(label_l_down, text="Ανακοινώσεις", command=lambda: raise_frame(intro_Frame), bg="SteelBlue3")
    butttonNext7 = Button(label_l_down, text="Προβλήματα", command=lambda: raise_frame(intro_Frame)
                          , bg="SteelBlue3")  # ,height = 2, width = 7
    butttonNext8 = Button(label_l_down, text="Έξοδος", command=lambda: raise_frame(intro_Frame)
                          , bg="SteelBlue3")  # ,height = 2, width = 7
